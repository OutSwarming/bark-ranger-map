...
| A03 | Google premium account | Switch Account, then Google free account | Free | normal URL | No | Chooser appears, app shows free account, premium locks, premium state cleared. | App-side chooser prompt is automated; free/premium distinct-state smoke passes. Real Google chooser and true same-browser Google Account A-to-B flow still need manual browser QC. | P0 | PARTIAL PASS | `account-auth-smoke.spec.js`, `phase3a-premium-gating-smoke.spec.js`; manual checklist pending |
| A04 | Google free account | Switch Account, then Google premium account | Premium | normal URL | No | Chooser appears, app shows premium account, premium unlocks after entitlement loads. | App-side chooser prompt is automated; premium storage state unlocks only after entitlement. Real Google chooser and true same-browser Google free-to-premium flow still need manual browser QC. | P0 | PARTIAL PASS | `account-auth-smoke.spec.js`, `phase3a-premium-gating-smoke.spec.js`; manual checklist pending |
...
- Required full e2e smoke with `free-user.json`, `free-user-b.json`, and `premium-user.json` still fails 15/16 because `free-user-b.json` has the same UID as `free-user.json`. The same smoke passes 16/16 when the second account is the distinct premium storage state.
...
- `playwright/.auth/free-user-b.json` still has the same UID as `playwright/.auth/free-user.json`, so true free/free isolation remains blocked by storage-state setup. Free-vs-premium distinct-account smoke passes.
...
- Regenerate `playwright/.auth/free-user-b.json` with a truly distinct free UID, then rerun account-switch isolation.

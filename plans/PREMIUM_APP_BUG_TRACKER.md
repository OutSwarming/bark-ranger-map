...
|---|---|---|---|---:|---:|---|---|---|---|---|---|---|
| BUG-001 | Gamification achievement Firestore permission/runtime issue | Firestore/rules/data sync | P1 | 5 | 4 | Post-deploy verification still fails: signed-in free and premium owner write/read to `users/{uid}/achievements/bug001RuntimeSmoke` returns `Missing or insufficient permissions.` Payload shape is exactly `achievementId`, `tier`, `dateEarned`. Local rules tests pass owner create/update/read and deny non-owner/unauth/dangerous fields, including the exact denied runtime UID/doc path. | Open Profile signed in, trigger achievement evaluation, then write/read owner achievement doc with allowed payload. Confirm no permission console error after rules deploy. | Active deployed `cloud.firestore` release for `barkrangermap-auth` is still ruleset `94384826-3a15-486f-af2d-613c752a77e6`, updated `2026-05-02T03:30:59Z`; its `firestore.rules` does not contain `match /achievements/{achievementId}` or `isValidAchievementWrite`. The reported deploy did not update the active Firestore rules release. | `firestore.rules`, `tests/rules/firestore-entitlement.rules.test.js`, `tests/playwright/bug001-achievement-permission-smoke.spec.js`, `gamificationLogic.js`, `profileEngine.js` | Repo rules fix exists in `576830f`; runtime smoke added in `17131f8` | FAIL after reported rules deploy: local rules PASS 17/17; focused runtime smoke FAIL 0/2; deployed ruleset lacks achievement rule | QC FAILED |
...
Root cause confirmed: The runtime path and payload match the repo rules contract, and local rules tests pass. The production runtime still denies owner achievement writes after the reported Firestore rules deploy because the active deployed `cloud.firestore` release for `barkrangermap-auth` is still using an older ruleset that does not contain the achievement subcollection rule.
...
Issue type: Repo rules/tests are correct; deployed Firestore rules do not match the repo rules in the runtime app. Do not patch the client to hide the error.
...
Exact behavior after reported rules deploy: No product code changed. The focused runtime smoke still captures the failure explicitly; both signed-in free and premium storage states can evaluate/render achievements, but direct owner achievement doc write/read is still denied by deployed Firestore rules.

Deploy record: User reported Firestore rules deploy approved and run before verification at 2026-05-03 03:27 EDT. Exact deploy output was not captured in this tracker.
...
Deployed rules evidence: `firestore:rules:get` is not available in Firebase CLI v14.26.0, so the Firebase Rules REST API was used read-only. Active release `projects/barkrangermap-auth/releases/cloud.firestore` points to ruleset `projects/barkrangermap-auth/rulesets/94384826-3a15-486f-af2d-613c752a77e6`, updated `2026-05-02T03:30:59.166809Z`; fetched ruleset content does not contain `match /achievements/{achievementId}` or `isValidAchievementWrite`.

Tests run after reported deploy: initial `npm run test:rules` PASS 16/16; BUG-001 focused achievement smoke FAIL 0/2 with `Missing or insufficient permissions` for free UID `LkevgscKPvPqRg9c5YKKXVqtwv02` and premium UID `6vrN6hQ8VQSzxvKRLuVdxWM2mpD2`; signed-in `npm run test:e2e:smoke` with free/free-b/premium storage states PASS 16/16; exact-path rules test added for `users/LkevgscKPvPqRg9c5YKKXVqtwv02/achievements/bug001RuntimeSmoke` and `npm run test:rules` PASS 17/17.

Deploy needed: Yes. The next command should be rules-only and explicit-project: `firebase deploy --only firestore:rules --project barkrangermap-auth`. Do not deploy functions.

Risk: Until the active deployed rules allow `users/{uid}/achievements/{achievementId}` owner writes, signed-in users may still see Firestore permission errors when the profile/share flow syncs achievement docs. The local rules remain narrow and continue to deny entitlement/payment/admin field writes.
...
QC Result: QC FAILED after reported Firestore rules deploy, local rules PASS. Status is `QC FAILED`.

Evidence: Static audit found no extra dangerous fields in achievement writes. Local rules tests prove owner-only achievement writes with the allowed payload are accepted and non-owner/unauth/dangerous writes are denied. Playwright runtime smoke proves the deployed app still denies the same owner write after the reported deploy. `.firebaserc` and the app config both target project `barkrangermap-auth`, and the read-only deployed rules fetch proves the active ruleset lacks the achievement rule.

Manual steps: User reported the Firestore rules deploy was run. No Firebase deploy was run during this verification pass.

Remaining risk: Need rerun the explicit-project rules deploy and verify the active release update time/ruleset content changes before rerunning `tests/playwright/bug001-achievement-permission-smoke.spec.js`; it should pass for free and premium accounts once production rules really match the repo.

Status update in tracker: BUG-001 is `QC FAILED` because the post-deploy runtime smoke still fails for both signed-in free and premium accounts, and the active deployed ruleset still lacks the achievement rule.

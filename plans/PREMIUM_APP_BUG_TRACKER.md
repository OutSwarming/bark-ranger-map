...
| BUG-015 | Free visited limit of 20 parks is not enforced | Premium product rules | P1 | 5 | 4 | User-reported product rule failure: signed-in free users can add more than 20 parks as visited. This blocks paid/controlled premium beta because core free/premium value boundary is incomplete. | Sign in as free user, reach 20 visited parks, attempt to mark a 21st official park visited via manual mark and GPS/check-in paths. | Visit-add paths may write directly through local/VaultRepo/Firebase state without consulting premium entitlement or visited-count policy. | `services`, `modules`, `repos`, `state`, `tests/playwright` |  | Pending audit/fix | FOUND |
| BUG-016 | Route generation premium gating/enforcement is unclear or incomplete | Premium product rules | P1 | 4 | 4 | User-reported route generation appears not grayed out or not clearly premium-only. Backend ORS callable tests pass, but UI/runtime pre-call gate and visual state need product-rule audit. | Sign in as free user, inspect route generation controls, attempt route action, confirm no ORS/callable call succeeds; repeat as premium. | Route controls may rely on backend rejection or visual copy only, or may not mirror premium entitlement state. | `modules`, `renderers`, `services`, `functions`, `tests/playwright` |  | Pending audit/fix | FOUND |
| BUG-017 | Premium product rules audit incomplete | Premium product rules | P1 | 5 | 4 | Gate report downgraded because auth/payment/security checks passed but product-tier rules need a full audit across visited limits, ORS/geocode, map styles, clustering, trails, checkout URLs, and entitlement writes. | Review `plans/PREMIUM_PRODUCT_RULES_AUDIT.md`; prove each paid surface has UI, runtime, and backend/rules enforcement as applicable. | Earlier bug hunt focused on entitlement leaks and runtime safety, not complete product-tier policy enforcement. | `plans/PREMIUM_PRODUCT_RULES_AUDIT.md`, `services`, `modules`, `functions`, `firestore.rules`, `tests` |  | Audit opened | FOUND |

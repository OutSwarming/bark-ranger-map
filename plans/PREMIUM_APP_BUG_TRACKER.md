...
|---|---|---|---|---:|---:|---|---|---|---|---|---|---|
| BUG-001 | Gamification achievement Firestore permission/runtime issue | Firestore/rules/data sync | P1 | 5 | 4 | Post-deploy verification still fails: signed-in free and premium owner write/read to `users/{uid}/achievements/bug001RuntimeSmoke` returns `Missing or insufficient permissions.` Payload shape is exactly `achievementId`, `tier`, `dateEarned`. Local rules tests pass owner create/update/read and deny non-owner/unauth/dangerous fields. | Open Profile signed in, trigger achievement evaluation, then write/read owner achievement doc with allowed payload. Confirm no permission console error after rules deploy. | Reported Firestore rules deploy did not resolve the production runtime denial; active deployed rules still appear not to include the repo's owner-only achievement subcollection allowance, despite `.firebaserc` and app config both targeting `barkrangermap-auth`. | `firestore.rules`, `tests/rules/firestore-entitlement.rules.test.js`, `tests/playwright/bug001-achievement-permission-smoke.spec.js`, `gamificationLogic.js`, `profileEngine.js` | Repo rules fix exists in `576830f`; runtime smoke added in `17131f8` | FAIL after reported rules deploy: local rules PASS 16/16; focused runtime smoke FAIL 0/2; full signed-in smoke PASS 16/16 | QC FAILED |
...
Issue type: Repo rules/tests are correct; deployed Firestore rules still do not appear to match the repo rules in the runtime app. Do not patch the client to hide the error.
...
Tests run after reported deploy: `npm run test:rules` PASS 16/16; BUG-001 focused achievement smoke FAIL 0/2 with `Missing or insufficient permissions` for free UID `LkevgscKPvPqRg9c5YKKXVqtwv02` and premium UID `6vrN6hQ8VQSzxvKRLuVdxWM2mpD2`; plain `npm run test:e2e:smoke` SKIP 16/16 because env vars were not set; signed-in `npm run test:e2e:smoke` with free/free-b/premium storage states PASS 16/16.

Deploy needed: Further deploy verification/retry appears needed, but no deploy was run in this verification pass. The expected rules-only command remains `firebase deploy --only firestore:rules`; do not deploy functions.
...
Remaining risk: Need verify the actual active deployed Firestore rules for `barkrangermap-auth` and rerun `tests/playwright/bug001-achievement-permission-smoke.spec.js`; it should pass for free and premium accounts without console permission errors once the production rules really match the repo.

Status update in tracker: BUG-001 is `QC FAILED` because the post-deploy runtime smoke still fails for both signed-in free and premium accounts.
